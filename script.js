...
function saveImage() {
  ...
  // Temporarily appling background
  quoteContainer.style.cssText +=
    "background: rgba(0,0,0,0.6); color: #fff; padding: 20px; border-radius: 10px;";

  html2canvas(quoteContainer, { backgroundColor: "#222", useCORS: true }).then(
    (canvas) => {
...
function updateNavButtons() {
  document.getElementById("prevBtn").style.display =
    currentQuoteIndex > 0 ? "block" : "none";
  document.getElementById("nextBtn").style.display =
    currentQuoteIndex < quotesHistory.length - 1 ? "block" : "none";
}
